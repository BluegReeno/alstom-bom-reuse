...
def test_the_artifacts_a_run_writes_are_named_once() -> None:
    """Seven files, said in one place: every test site that lists them derives its list from here."""
    assert RUN_ARTIFACTS == ("normalized.json", "resolution.json", "note_facts.json", "findings.json", "signatures.json", "predictions.json", "report.html")
